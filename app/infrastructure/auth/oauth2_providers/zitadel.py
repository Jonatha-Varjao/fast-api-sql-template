...
class ZitadelProvider(OAuth2ProviderBase):
    def __init__(self):
        self.server_url = settings.oauth2_server_url
        self.client_id = settings.oauth2_client_id
        self._jwks_uri = f"{self.server_url}/oauth/v2/keys"
        self._userinfo_endpoint = f"{self.server_url}/oauth/v2/userinfo"
    ...
    async def validate_token(self, token: str) -> OAuth2UserInfo:
        ...
        return OAuth2UserInfo(
            ...
            is_superuser=payload.get("roles", []).count("admin") > 0,
